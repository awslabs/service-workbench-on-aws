...
describe('WorkflowDraftService', () => {
  ...
  it('should throw error because Step Function cannot execute state machine', async () => {
    ...
    const params = {
      stateMachineArn: 'arn:aws:states:eu-west-3:111111111111:stateMachine:abc',
      input:
        '{"meta":{"workflowId":"wf-start-ec2-environment-sc","workflowVer":1,"smWorkflow":"arn:aws:states:eu-west-3:111111111111:stateMachine:abc","wid":"wf-start-ec2-environment-sc","sid":"BCigolBoG4svWa0uiZTR9","wrv":1},"input":{"environmentId":"1234567","instanceIdentifier":"some-ec2-instance-id","cfnExecutionRole":"cfnExecutionRole","roleExternalId":"roleExternalId"}}',
    ...
    await expect(workflowTriggerService.triggerWorkflow(requestContext, meta, input)).rejects.toThrow(
      `Step Function could not start execution for State Machine arn:aws:states:eu-west-3:111111111111:stateMachine:abc with params ${JSON.stringify(
        params,
