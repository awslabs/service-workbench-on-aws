...
const { getAuth0TokenVerifier } = require('./auth0-token-verifier');
...
class ProviderService extends Service {
  ...
  async validateToken({ token, issuer }, providerConfig) {
    ...
    const { username, identityProviderName } = await this.createUserIfDoesntExist(
      verifiedToken,
  ...
  async createUserIfDoesntExist(decodedToken, authenticationProviderId, identityProviderName) {
    // IdProviders) {
