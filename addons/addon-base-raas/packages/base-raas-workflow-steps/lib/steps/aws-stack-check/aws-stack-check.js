...
class AwsStackCheck extends StepBase {
  async start() {
    this.print('start pinging aws account stack');

    // Get services
    const awsCfnService = await this.mustFindServices('AwsCfnService');
    ...
    const awsAccountIds = await this.payload.object('awsAccountId');

    const processor = async awsAccountId => {
      await awsCfnService.attemptReach(requestContext, { id: awsAccountId, type: 'awsAccount' }, { forceCheckAll });
    };

    // For each dsAccount, reach out 10 at a time
    await processInBatches(awsAccountIds, 10, processor);
  }
}

module.exports = AwsStackCheck;
