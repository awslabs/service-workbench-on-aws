...
class BudgetsService extends Service {
  ...
  async init() {
    ...
    this.aws = null;
    this.awsAccountsService = null;
    this.jsonSchemaValidationService = null;
  }
  ...
  async validateInput(requestBody) {
    ...
    await this.jsonSchemaValidationService.ensureValid(requestBody, createSchema);
  ...
  async _initServices() {
    this.awsAccountsService = await this.service('awsAccountsService');
  ...
  async _getBudgetClientAndAWSAccountId(requestContext, id) {
    ...
    const budgetsClient = new this.aws.sdk.Budgets({
  ...
  async _getCredentials(requestContext, awsAccountUUID) {
    const { roleArn, externalId, accountId } = await runAndCatch(
      async () => {
        return this.awsAccountsService.mustFind(requestContext, { id: awsAccountUUID }); // awsAccountId
      },
      async () => {
    ...
    const { accessKeyId, secretAccessKey, sessionToken } = await this.aws.getCredentialsForRole({
      roleArn,
