// This code is the same encryption algorithm used by RStudio's UI
...
let g;
let j;
const k = (244837814094590 & 16777215) == 15715070;
function l(b, a, c) {
  if (b != null)
    if (typeof b === 'number') this.ca(b, a, c);
    else a == null && typeof b !== 'string' ? this.z(b, 256) : this.z(b, a);
}
function m() {
  return new l(null);
}
...
function B(b) {
  for (let a = this.a - 1; a >= 0; --a) b[a] = this[a];
  b.a = this.a;
  b.b = this.b;
}
function D(b) {
...
function H() {
  for (let b = this.b & this.g; this.a > 0 && this[this.a - 1] == b; ) --this.a;
}
function I(b) {
...
function ba(b, a) {
  for (let c = b; c < this.a; ++c) a[c - b] = this[c];
...
function ha(b, a, c) {
  ...
  if (!(d.a <= 0)) {
    ...
    if (e.a < d.a) {
      ...
      e = f[d - 1];
      if (e != 0) {
        let n = e * (1 << this.r) + (d > 1 ? f[d - 2] >> this.s : 0);
        const r = this.K / n;
        n = (1 << this.r) / n;
        const ia = 1 << this.s;
        let w = c.a;
        ...
        for (; --y >= 0; ) {
          let C = c[--w] == e ? this.g : Math.floor(c[w] * r + (c[w - 1] + ia) * n);
...
function sa(b) {
  ...
  for (let a = 0; a < this.d.a; ++a) {
    let c = b[a] & 32767;
    const d = (c * this.C + (((c * this.T + (b[a] >> 15) * this.C) & this.Y) << 15)) & b.g;
    c = a + this.d.a;
...
if (U == null) {
  ...
  if (navigator.appName == 'Netscape' && navigator.appVersion < '5' && window.crypto) {
    const Y = window.crypto.random(32);
...
// Parameters are payload, exponent, and modulus
module.exports.encrypt = function(b, a, c) {
  let d = new $();
  d.X(c, a);
  b = d.N(b);
  d = '';
  for (a = 0; a + 3 <= b.length; a += 3) {
    ...
    d +=
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.charAt(c >> 6) +
  ...
  if (a + 1 == b.length) {
    ...
    d +=
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.charAt(c >> 2) +
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.charAt((c & 3) << 4);
  }
  for (; (d.length & 3) > 0; ) d += '=';
  return d;
};
