...
/**
 * A function to get cross account CloudFormation outputs. The cloudFormationSettings parameter
 * contains details of the CloudFormation outputs to retrieve, and the targetAwsProfile is the
 * profile from which those outputs will be pulled:
 *
 * {
 ...
 *
 * @param stage
 * @param targetAwsProfile
 * @param awsRegion
 * @param currentSettings
 * @param cloudFormationSettings
 * @returns {Promise<{*}>}
 */
async function getCloudFormationCrossAccountValues(
  stage,
  targetAwsProfile,
  awsRegion,
  currentSettings,
  ...
  const cf = createAwsSdkClient('CloudFormation', targetAwsProfile, { apiVersion: '2010-05-15', region: awsRegion });

  // Pull in CloudFormation output values from the target account
  const settingsForExpansion = { ...currentSettings, stage, awsRegion };
...
exports.getCloudFormationCrossAccountValues = getCloudFormationCrossAccountValues;
