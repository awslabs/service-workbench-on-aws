...
class UpdateBudget extends React.Component {
  constructor(props) {
    ...
    runInAction(() => {
      // Need parameter budgetExist to determine if notification if editable
      // Will remove once notification update is complete
      this.budgetExist = false;
    });
  }
  ...
  renderMain() {
    ...
    runInAction(() => {
      ...
      this.budgetExist = budget.budgetLimit !== "";
    });
        ...
        {({ processing, onCancel }) => (
          <>
            ...
            <DropDown
              ...
              clearable
              fluid
              // disabled={this.budgetExist}
            />
            <Input field={form.$('notificationEmail')} type="email" /* disabled={this.budgetExist} */ />
            <div className="mt3">
              <Button
                floated="right"
                primary
