...
describe("start-image-builder", () => {
  ...
  describe("createImageBuilder", () => {
    test("start Image Builder successfully", async () => {
      ...
      appStreamMock.on(CreateImageBuilderCommand).resolves();
    ...
    test("failed to get default vpc", async () => {
      ...
      const ec2Mock = mockClient(EC2Client);
      ec2Mock.on(DescribeVpcsCommand).rejects("Describe VPCs failed");

      await expect(startImageBuilder.createImageBuilder()).rejects.toEqual(
        new Error("Describe VPCs failed")
      );
    });
  });
  ...
  describe("waitForImageBuilderToBeReady", () => {
    ...
    test("Image Builder failed to transition to RUNNING state", async () => {
      ...
      // OPERATE, CHECK
      await expect(
        startImageBuilder.waitForImageBuilderToBeReady()
      ).rejects.toEqual(new Error("DescribeImageBuilder failed"));
    }, 10000);
  });
});
