...
describe('Create keypair scenarios', () => {
  ...
  describe('Get key pair', () => {
    it('should fail for anonymous user', async () => {
      ...
      await expect(anonymousSession.resources.keyPairs.get()).rejects.toMatchObject({
    ...
    it.each(userRoles)('should fail for inactive %userRole', async userRole => {
      ...
      await expect(userSession.resources.keyPairs.get()).rejects.toMatchObject({
    ...
    it.each(userRoles)('should succeed for %userRole', async userRole => {
      const userSession = await setup.createUserSession({ userRole, projectId: [] });
      await userSession.resources.keyPairs.create();
      await expect(userSession.resources.keyPairs.get()).resolves.toHaveLength(1);
    });
  });
});
