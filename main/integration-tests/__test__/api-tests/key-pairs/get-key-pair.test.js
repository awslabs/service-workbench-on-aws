...
describe('Get key pair', () => {
  ...
  describe('Get key pair', () => {
    it('should fail for anonymous user', async () => {
      ...
      await expect(anonymousSession.resources.keyPairs.keyPair(keyPair.id).get()).rejects.toMatchObject({
    ...
    it.each(userRoles)('should fail for inactive %userRole', async userRole => {
      ...
      await expect(userSession.resources.keyPairs.keyPair(userKeyPair.id).get()).rejects.toMatchObject({
    ...
    it.each(userRoles)('should succeed for %userRole', async userRole => {
      const userSession = await setup.createUserSession({ userRole, projectId: [] });
      const userKeyPair = await userSession.resources.keyPairs.create();
      await expect(userSession.resources.keyPairs.keyPair(userKeyPair.id).get()).resolves.toMatchObject({
        createdBy: userSession.user.uid,
      });
    });

    it.each(nonAdminUserRoles)(
      'should fail for non-admin user %userRole getting key pair of another user',
      async userRole => {
        const userSession = await setup.createUserSession({ userRole, projectId: [] });
        const adminKeyPair = await adminSession.resources.keyPairs.create();
        await expect(userSession.resources.keyPairs.keyPair(adminKeyPair.id).get()).rejects.toMatchObject({
