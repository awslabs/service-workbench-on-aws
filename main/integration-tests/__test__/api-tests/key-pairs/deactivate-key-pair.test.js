...
describe('Get key pair', () => {
  ...
  describe('Deactivate key pair', () => {
    it('should fail for anonymous user', async () => {
      const keyPair = await adminSession.resources.keyPairs.create();
      const anonymousSession = await setup.createAnonymousSession();
      await expect(
        anonymousSession.resources.keyPairs.keyPair(keyPair.id).deactivate({ rev: 0 }),
    ...
    it.each(userRoles)('should fail for inactive %userRole', async userRole => {
      ...
      await expect(userSession.resources.keyPairs.keyPair(userKeyPair.id).deactivate({ rev: 0 })).rejects.toMatchObject(
        {
    ...
    it.each(userRoles)('should succeed for %userRole', async userRole => {
      ...
      const userKeyPair = await userSession.resources.keyPairs.create();
      await expect(
        userSession.resources.keyPairs.keyPair(userKeyPair.id).deactivate({ rev: 0 }),
    ...
    it.each(nonAdminUserRoles)(
      'should fail for non-admin user %userRole deactivating key pair of another user',
      async userRole => {
        const userSession = await setup.createUserSession({ userRole, projectId: [] });
        const adminKeyPair = await adminSession.resources.keyPairs.create();
        await expect(
          userSession.resources.keyPairs.keyPair(adminKeyPair.id).deactivate({ rev: 0 }),
        ).rejects.toMatchObject({
