...
describe('Approve workspace-type scenarios', () => {
  ...
  describe('Approve workspace-type', () => {
    it('should fail if user is inactive', async () => {
      ...
      await expect(
        adminSession2.resources.workspaceTypes.workspaceType(workspaceTypeId).approve(approveBody),
    ...
    it('should fail if user is not admin', async () => {
      ...
      await expect(
        researcherSession.resources.workspaceTypes.workspaceType(workspaceTypeId).approve(approveBody),
      ).rejects.toMatchObject({
    ...
    it('should fail if user is anonymous', async () => {
      ...
      await expect(
        anonymousSession.resources.workspaceTypes.workspaceType(workspaceTypeId).approve(approveBody),
      ).rejects.toMatchObject({
    ...
    it('should fail if input is not valid', async () => {
      ...
      await expect(
        adminSession.resources.workspaceTypes.workspaceType(workspaceTypeId).approve(approveBody),
      ).rejects.toMatchObject({
    ...
    it('should approve if user is admin', async () => {
      ...
      await expect(
        adminSession.resources.workspaceTypes.workspaceType(workspaceTypeId).approve(approveBody),
      ).resolves.toHaveProperty('status', 'approved');
    });
  });
});
