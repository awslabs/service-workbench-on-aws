...
describe('Update budget', () => {
  ...
  beforeAll(async () => {
    ...
    budgetObject = adminSession.resources.budgets.defaults({ id: accountUUID });
  });
  ...
  describe('Update budget', () => {
    ...
    it.each(userRoles)('should fail for inactive %userRole', async userRole => {
      ...
      await expect(userSession.resources.budgets.update({ id: accountUUID })).rejects.toMatchObject({
    ...
    it('should succeed for admin', async () => {
      ...
      await expect(userSession.resources.budgets.update(budgetObject)).resolves.toMatchObject({
