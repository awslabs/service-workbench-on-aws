...
describe('Get authentication types list scenarios', () => {
  ...
  describe('Get authentication types list', () => {
            ...
            config: {
              ...
              inputSchema: {
                ...
                properties: {
                  ...
                  enableNativeUserPoolUsers: { $id: '#/properties/enableNativeUserPoolUsers', type: 'boolean' },
                  federatedIdentityProviders: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        id: { $id: '#/properties/federatedIdentityProviders/properties/id', type: 'string' },
                        metadata: {
                          $id: '#/properties/federatedIdentityProviders/properties/metadata',
                          type: 'string',
                        },
                        name: { $id: '#/properties/federatedIdentityProviders/properties/name', type: 'string' },
                      },
    ...
    it('should fail if internal guest attempts to get auth types list', async () => {
      const { session } = await setup.createUserSession({ userRole: 'internal-guest', projectId: [] });

      await expect(session.resources.authentication.types().get()).rejects.toMatchObject({
        code: errorCode.http.code.forbidden,
      });
    });

    it('should fail if external guest attempts to get auth types list', async () => {
      const { session } = await setup.createUserSession({ userRole: 'guest', projectId: [] });
