...
describe('Delete workflow draft scenarios', () => {
  let setup;
  let adminSession;
  ...
  describe('Deleting a workflow  draft', () => {
    ...
    it('should delete the workflow draft for admin', async () => {
      const draft = await adminSession.resources.workflows.drafts().create();
      ...
      await expect(adminSession.resources.workflows.drafts().find(draft.id)).resolves.not.toBeDefined();
    });
  });
});
