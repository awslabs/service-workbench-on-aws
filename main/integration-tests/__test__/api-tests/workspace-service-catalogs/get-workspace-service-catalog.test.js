...
describe('Get workspace-service-catalog scenarios', () => {
  ...
  describe('Get workspace-service-catalog', () => {
    it('should fail if user is inactive', async () => {
      ...
      await expect(
        adminSession2.resources.workspaceServiceCatalogs.workspaceServiceCatalog(response.id).get(),
    ...
    it('should fail if user is anonymous', async () => {
      ...
      await expect(
        anonymousSession.resources.workspaceServiceCatalogs.workspaceServiceCatalog(response.id).get(),
      ).rejects.toMatchObject({
    ...
    it('should fail if user role is not owner', async () => {
      ...
      await expect(
        researcherSession.resources.workspaceServiceCatalogs.workspaceServiceCatalog(response.id).get(),
      ).rejects.toMatchObject({
    ...
    it('should pass if user is admin', async () => {
      ...
      await expect(
        adminSession.resources.workspaceServiceCatalogs.workspaceServiceCatalog(response.id).get(),
      ).resolves.toMatchObject({
        envTypeId: workspaceTypeId,
        envTypeConfigId: configurationId,
      });
